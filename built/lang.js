...
var Lang = /** @class */ (function () {
    function Lang(options) {
        ...
        this.fallback = null;
        this.messages = null;
        this.locale = options.locale || utils_1.inferLocale() || DEFAULT_LOCALE;
        this.fallback = options.fallback;
        this.setMessages(options.messages);
    }
    Lang.prototype.setMessages = function (messages) {
        this.messages = utils_1.flat(messages);
    };
    ...
    Lang.prototype.get = function (key, replacements, locale) {
        ...
        var message = lodash_1.get(this.messages, source, null);
        if (message === null) {
            message = lodash_1.get(this.messages, sourceFallback, null);
            if (message === null) {
                return key;
            }
        }
        if (replacements) {
            return utils_1.applyReplacements(message, replacements);
        }
        return message;
    };
    ...
    Lang.prototype.choice = function (key, number, replacements, locale) {
        ...
        for (var j = 0; j < explicitRules.length; j++) {
            if (utils_1.testInterval(number, explicitRules[j])) {
                return messageParts[j];
            }
        }
        locale = locale || this._getLocaleFromExistingMessage(key, locale);
        var pluralForm = utils_1.getPluralForm(number, locale);
