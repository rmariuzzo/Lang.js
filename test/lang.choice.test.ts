...
describe('The lang.choice() method', () => {
  ...
  it('should return the expected message with different count', () => {
    lang.setMessages({
      'en.plural': {
  ...
  it('should return the expected message using math intervals', () => {
    ...
    expect(lang.choice('test.infinityStar', -Infinity)).toBe(
      '-Infinity Negative'
    )
  ...
  it('should return the expected message with replacements', () => {
    expect(lang.choice('validation.accepted', 1)).toBe(
      'The :attribute must be accepted.'
    )
    expect(
      lang.choice('validation.accepted', 1, {
        attribute: 'foo'
      })
  ...
  it('should return the expected message using the fallback language', () => {
    ...
    expect(lang.choice('messages.box', 1)).toBe('1 box')
    expect(lang.choice('messages.box', 2)).toBe('2 boxes')
    expect(lang.choice('messages.box', 10)).toBe('10 boxes')
  })
})
